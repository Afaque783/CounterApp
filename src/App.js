import { useState } from "react";
import "./App.css";
import { set } from "mongoose";

function App() {
  const[count,setCount] = useState(0);
  function decreaseHandler(){
    setCount(count-1);
  }
  function increaseHandler(){
    setCount(count+1);
  }
  function resetHandler(){
    setCount(0);
  }
  return (
    <div className="w-[100vw] h-[100vh] flex justify-center items-center flex-col gap-10 bg-[#344151]">
      <div className="font-medium text-2xl text-[#0398d4]">Increement And Decreement</div>
      <div className="flex justify-center gap-12 py-3 rounded-sm text-[25px] bg-white text-[#344151]">
        <button onClick={decreaseHandler} className="border-r-2 text-center text-5xl w-20 border-[#bfbfbf]">
          -
        </button>
        <div className="font-bold gap-12 text-5xl">
          {count}
        </div>
        <button onClick={increaseHandler} className="border-l-2 text-center text-5xl w-20 border-[#bfbfbf]">
          +
        </button>
      </div>
      <button onClick={resetHandler} className="px-5 py-2 rounded-sm text-lg text-white bg-[#0398d4]">
        Reset
      </button>
    </div>
  );
}

export default App;
